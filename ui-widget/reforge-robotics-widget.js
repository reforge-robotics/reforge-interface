(() => {
  ...
  template.innerHTML = `
    ...
    <button class="launcher" type="button" aria-expanded="false" aria-label="Enable Precision Control">
      <span class="launcherLabel">Enable Vibration Control</span>
      <span class="launcherIcon" aria-hidden="true">&#9656;</span>
    </button>
    <div class="widget hidden" part="widget">
      <div class="widgetTop">
        <div class="poweredBy">Powered by Reforge</div>
        <button class="close" type="button" aria-label="Close vibration control">&times;</button>
      </div>
      <div class="statusRow">
      ...
      <div class="section registeredOnly">
        ...
        <div class="calibrateHint hidden">Connect robot before calibration</div>
        <div class="connectionStatus hidden" aria-live="polite"></div>
      </div>
  ...
  class ReforgeRoboticsWidget extends HTMLElement {
    ...
    _bind() {
      ...
      this._elements.launcher.addEventListener("click", () => {
      ...
      this._elements.updateId.addEventListener("click", () => {
        const isEditing = this._isEditing || !this.robotId;
        if (isEditing) {
      ...
      this._elements.connectTest.addEventListener("click", () => {
        ...
        this._render();
        this.dispatchEvent(
          new CustomEvent("reforge:connectTest", {
            detail: {},
            bubbles: true,
            composed: true
          })
    ...
    _render() {
      ...
      const connectionMessage = this.connectionMessage;
      ...
      this._elements.launcher.classList.toggle("hidden", this._isExpanded);
      this._elements.launcher.setAttribute("aria-expanded", String(this._isExpanded));
      this._elements.root.classList.toggle("hidden", !this._isExpanded);
      this._elements.root.classList.toggle("busy", busy);

      if (this._elements.connectionStatus) {
        this._elements.connectionStatus.textContent = connectionMessage;
        this._elements.connectionStatus.classList.toggle("hidden", !connectionMessage);
        this._elements.connectionStatus.classList.remove("success", "error", "running");
        if (connectionStatus === "success" || connectionStatus === "error" || connectionStatus === "running") {
          this._elements.connectionStatus.classList.add(connectionStatus);
        }
      }
    ...
    /**
     * Applies connecting state to the Connect button.
     * Side effects: toggles button disabled state and label.
     */
    ...
    /**
     * @returns {boolean} Whether connect-test is in progress.
     */
    get connecting() {
      return this._parseBooleanAttr("connecting");
    }

    /**
     * @param {boolean} value - connect-test state.
     */
    ...
    /**
     * @returns {string} Connection status marker: success|error|running.
     */
    get connectionStatus() {
      return this.getAttribute("connection-status") || "";
    }

    /**
     * @param {string} value - connection status marker.
     */
    set connectionStatus(value) {
      if (!value) {
        this.removeAttribute("connection-status");
      } else {
        this.setAttribute("connection-status", value);
      }
    }

    /**
     * @returns {string} Connection status message text.
     */
    get connectionMessage() {
      return this.getAttribute("connection-message") || "";
    }

    /**
     * @param {string} value - connection status message.
     */
    set connectionMessage(value) {
      if (!value) {
        this.removeAttribute("connection-message");
      } else {
        this.setAttribute("connection-message", value);
      }
    }
  }
