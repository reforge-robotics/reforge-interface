...
usage() {
  echo "Usage: $0 <identify_api_token> <robot_id> <data_folder>"
...
CONTAINER_NAME="${CONTAINER_NAME:-reforge-identify}"
...
docker run --net=host --rm --name "$CONTAINER_NAME" \
  ...
  "$IMAGE_NAME" \
  python3 -m robot.run identify "$identify_api_token" "$robot_id" "$CONTAINER_DATA_DIR"
